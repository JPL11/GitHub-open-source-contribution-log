...
**Status:** ✅ Merged (June 12, squash commit
[`d0b9c0338`](https://github.com/WebAssembly/binaryen/commit/d0b9c033808fc85640a240819d74f8347b769155));
issue #3244 closed
...
- [x] Covered-region merging (added at reviewer request): segments shaped like
  A: [3,8), B: [1,2), C: [0,5) — without merging B's region into C's, the map
  lookup when visiting A would find B and miss the overlap with C
...
### Review Round (June 12)

tlively (the CODEOWNERS-assigned reviewer) reviewed with "Very nice! Thanks
for the contribution." and three requests, all addressed in `2738c3896`:

1. **TODO for the imported-memory gate** — added a TODO noting we could
   optimize imported memories too if all segments after the trampled one, up
   to and including the trampling one, are provably in-bounds (so no trap can
   land between the two writes). Matches the follow-up I'd flagged in the PR
   description.
2. **Blank lines between test modules** — readability fix across both test
   files, matching the file's existing convention.
3. **A test that depends on covered-region merging** — tlively proposed the
   exact adversarial shape (A: [3,8), B: [1,2), C: [0,5)); implemented as
   `"abcde"`@1027, `"B"`@1025, `"fghij"`@1024. The generated CHECK lines prove
   the merge works: `"abcde"` shrinks to `"cde"`@1029, `"B"` is removed.

Replied on each review thread, re-ran `update_lit_checks.py`, both lit tests
pass locally. Approved same day: **"LGTM, thanks!"**

Also survived some CI infrastructure drama on June 11: the macos-14 job failed
*after* a successful build because GitHub's artifact-upload API timed out,
which fail-fast-cancelled both Windows jobs mid-test-suite. Diagnosed from the
job logs (no relation to the change), retriggered with an empty commit, and
left a PR comment so reviewers wouldn't read it as a real failure. Fork PRs
from first-time contributors also need a maintainer to approve each CI run —
worth knowing for the next contribution.
...
- **Key commits:**
  ...
  [`2738c3896`](https://github.com/JPL11/binaryen/commit/2738c3896) (review
  feedback: TODO, test formatting, covered-region merging test)
...
**PR Link:** https://github.com/WebAssembly/binaryen/pull/8834
...
**Maintainer Feedback:**
- June 11: PR opened; commented tagging @kripken (issue triager) and flagged the
  imported-memory gate as the main review question. CODEOWNERS auto-requested
  review from @tlively.
- June 11: CI failed on infrastructure (macos-14 artifact-upload timeout;
  Windows jobs fail-fast-cancelled). Diagnosed from logs, retriggered via empty
  commit, posted an explanatory PR comment.
- June 12: @tlively reviewed — "Very nice! Thanks for the contribution." —
  with three requests (TODO on the imported-memory gate, blank lines between
  test modules, a covered-region-merging test). Addressed in `2738c3896` and
  replied on each thread (see Review Round above).
- June 12: @tlively **approved**: "LGTM, thanks!"
- June 12: CI green on `2738c3896`; @tlively squash-merged the PR as
  [`d0b9c0338`](https://github.com/WebAssembly/binaryen/commit/d0b9c033808fc85640a240819d74f8347b769155)
  and issue #3244 closed automatically.

**Status:** ✅ Merged into `main`; issue closed. Contribution complete.
